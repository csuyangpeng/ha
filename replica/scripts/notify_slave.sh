...
docker exec -it mysql -uroot -p's<9!Own1z4' -h 127.0.0.1 -e "RESET MASTER;" 2>> $LOG

# 4. 指向新的主库（通过VIP，确保始终连接到当前活动主库）
docker exec -it mysql -uroot -p's<9!Own1z4' -h 127.0.0.1 <<EOF 2>> $LOG
CHANGE MASTER TO
...
# 5. 启动复制
docker exec -it mysql -uroot -p's<9!Own1z4' -h 127.0.0.1 -e "START SLAVE;" 2>> $LOG

# 6. 确保本机处于只读状态
docker exec -it mysql -uroot -p's<9!Own1z4' -h 127.0.0.1 -e "SET GLOBAL read_only=ON;" 2>> $LOG

# 7. 检查复制状态
echo "$(date): [NOTIFY] Checking slave status..." >> $LOG
docker exec -it mysql -uroot -p's<9!Own1z4' -h 127.0.0.1 -e "SHOW SLAVE STATUS\G" 2>> $LOG | grep -E "Slave_IO_Running|Slave_SQL_Running|Last_Error" >> $LOG
