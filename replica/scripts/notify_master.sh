...
docker exec -it mysql -u$MYSQL_USER -p$MYSQL_PASS -h 127.0.0.1 -e "SET GLOBAL read_only=OFF;" 2>> $LOG
...
docker exec -it mysql -u$MYSQL_USER -p$MYSQL_PASS -h 127.0.0.1 -e "FLUSH BINARY LOGS;" 2>> $LOG
