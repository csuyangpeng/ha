...
echo "[REDIS NOTIFY] Becoming BACKUP. Configuring as SLAVE..." 2>&1 | _ts_pipe >> "$LOG"

# 获取当前 Redis 角色
ROLE=$(docker exec -i redis redis-cli -a "A8K5h7+6!?" info replication 2>/dev/null | grep "role:" | cut -d: -f2 | tr -d '\r')

if [ "$ROLE" = "master" ]; then
    echo "Current role is MASTER, demoting to SLAVE of $MASTER_IP..." 2>&1 | _ts_pipe >> "$LOG"
    
    # 配置为指定主库的从库
    docker exec -i redis redis-cli -a "A8K5h7+6!?" slaveof $MASTER_IP 6379 2>&1 | _ts_pipe >> "$LOG"
    ...
    if [ "$CURRENT_MASTER" != "$MASTER_IP" ]; then
        echo "Switching master from $CURRENT_MASTER to $MASTER_IP" 2>&1 | _ts_pipe >> "$LOG"
        docker exec -i redis redis-cli -a "A8K5h7+6!?" slaveof $MASTER_IP 6379 2>&1 | _ts_pipe >> "$LOG"
    else
        echo "Already SLAVE of correct master $MASTER_IP" 2>&1 | _ts_pipe >> "$LOG"
    fi
fi

echo "[REDIS NOTIFY] BACKUP transition complete" 2>&1 | _ts_pipe >> "$LOG"