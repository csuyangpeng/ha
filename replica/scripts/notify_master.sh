...
# 1. 停止复制线程
mysql -uroot -p's<9!Own1z4' -h 127.0.0.1 -e "STOP SLAVE;" 2>> $LOG

# 2. 重置从库状态，解除与原主的关系
mysql -uroot -p's<9!Own1z4' -h 127.0.0.1 -e "RESET SLAVE ALL;" 2>> $LOG

# 3. 【重要】确保本机可写，成为主库。这是最关键的一步。
mysql -uroot -p's<9!Own1z4' -h 127.0.0.1 -e "SET GLOBAL read_only=OFF;" 2>> $LOG
...
mysql -uroot -p's<9!Own1z4' -h 127.0.0.1 -e "FLUSH BINARY LOGS;" 2>> $LOG
